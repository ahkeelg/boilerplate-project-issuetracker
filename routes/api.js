...
const filters = (query, pIndex) => {
  let {assigned_to, status_text, open, _id, issue_title, issue_text, created_by, created_on, updated_on} = query;
    ...
    if(typeof values == "object"){
      for(let i of values){
        for(let j in data[pIndex].issues){
          if(data[pIndex].issues[j][keys].toString() == i){
            if(issuePos.filter(obj => obj._id == data[pIndex].issues[j]._id).length == 0){
      ...
      for(let i in data[pIndex].issues){
        if(data[pIndex].issues[i][keys].toString() == values){
          if(issuePos.filter(obj => obj._id == data[pIndex].issues[i]._id).length == 0){
